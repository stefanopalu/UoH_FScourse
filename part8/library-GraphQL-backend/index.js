...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
        ...
        if (args.author) {
          searchedBooks = searchedBooks.filter(book => book.author === args.author)
        }
        if (args.genre){
           searchedBooks = searchedBooks.filter(book => book.genres.includes(args.genre))
        }
  ...
  Mutation: {
    addBook: (root, args) => {
        if (books.find(book => book.title === args.title && book.author === args.author)) {
            throw new GraphQLError('This book is already in the database', {
              extensions: {
                code: 'BAD_USER_INPUT',
